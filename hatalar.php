...
<div class="content">
    <h2 class="d-print-none">Sistem Hata Mesajları</h2>
    ...
    <thead>
        <tr>
            ...
            <th style="font-size:12px">Istenilen URL</th>
			<th style="font-size:12px">Durum Kodu</th>
	...
	<?php 
		...
		if ($admin == 1){
			...
			while($user = mysqli_fetch_assoc($result)) {
				...
				echo '<td>'.$user["requestedURL"].'</td>';
				echo '<td>'.$user["statusCode"].'</td>';
				echo '<td>'.kk($user["date"]).'</td>';
				echo '<td>'.$user["ip"].'</td>';
